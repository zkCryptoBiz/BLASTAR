...
function upateShare(score) {
  const tweetIntent = `https://twitter.com/intent/post?text=I%20just%20got%20${score}%20points%20on%20https%3A%2F%2Fblastar.wtf%20%40blastarsolana%20%F0%9F%91%BE%20%40elonmusk%E2%80%99s%20first%20game`;
  document.getElementById("share-tweet").setAttribute("href", tweetIntent);
...
function maybeGiveInstructions(keycode) {
  ...
  if (keycode == 121 || keycode == 89) {
    ...
    print("");
    print("        <       USE JOYSTICK      >");
    print(""); // Removed 2 spaces.
    print("               FOR CONTROL AND");
    print("");
    print("                 FIRE BUTTON");
    print("");
    print("                  TO SHOOT");
    ...
    print("");
    print("");
    print("    MISSION:DESTROY ALIEN FREIGHTER");
    print("");
    print("    CARRYING DEADLY HYDROGEN BOMBS");
    print("");
    print("    AND STATUS BEAM MACHINES");
    print("");
...
function redraw() {
  ...
  for (var i = 0; i < sprite.length; i++) {
    if (sprite[i])
      ctx.drawImage(sprite[i], spriteX[i] * 3, spriteY[i] * 3, 24, 24);
  }
}
    ...
    for (var j = i + 1; j < 6; j++) {
      ...
      if (Math.abs(xi - xj) < 8 && Math.abs(yi - yj) < 8) {
        for (var y = Math.max(yi, yj); y < Math.min(yi + 8, yj + 8); y++) {
          if (xj < xi) {
            if ((spriteBits[i][y - yi] << (xi - xj)) & spriteBits[j][y - yj])
              collision();
          } else {
            if ((spriteBits[j][y - yj] << (xj - xi)) & spriteBits[i][y - yi])
              collision();
          }
        }
      }
...
function print(txt) {
  ...
  textBuffers[cursorRow] =
    pre + txt + oldText.substr(cursorCol + pre.length + txt.length);
  cursorRow++;
  cursorCol = 0;
  redraw();
}
...
function delayRelease(callback) {
  setTimeout(
    callback,
    Math.max(0, kMainLoopMillis - (Date.now() - lastKeyDownMillis))
  );
}

function fakeKeyDown(keyCode) {
  if (keyPressCallback == null)
    keyDown({ keyCode: keyCode, preventDefault: function () {} });
}
...
function wavHeader(n) {
  return (
    "RIFF" +
    encodeLong(36 + 2 * n) +
    "WAVEfmt \x10\x00\x00\x00\x01\x00\x01\x00" +
    encodeLong(samplingRate) +
    encodeLong(2 * samplingRate) +
    "\x02\x00\x10\x00data" +
    encodeLong(2 * n)
  );
}
